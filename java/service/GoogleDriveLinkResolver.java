...
public class GoogleDriveLinkResolver {
    ...
    // Regex patterns for confirmation tokens (adapted from Python)
    // For confirm token: looks for confirm=TOKEN or name="confirm" value="TOKEN"
    ...
    public DownloadItem resolveDriveUrl(String pageUrl) {
        ...
        return attemptDownload(initialDownloadUrl, fileId, 0); // Start attempt, depth 0
    }
    ...
    private DownloadItem attemptDownload(String downloadUrl, String fileId, int attemptDepth) {
        ...
        if (attemptDepth > 2) { // Limit recursion (initial + one confirmation attempt)
        ...
        try {
            ...
            // HttpURLConnection handles cookies automatically due to CookieHandler.setDefault
            connection.setInstanceFollowRedirects(true); // Important for Drive links
            connection.setConnectTimeout(20000); // Increased timeout
            ...
            if (contentDisposition != null && !contentDisposition.isEmpty()) {
                ...
                return new DownloadItem(filename, connection.getURL().toString(), fileSize, null); // gofileContentId is null
            }

            // If no Content-Disposition, it might be a confirmation page or an error page
            ...
            if (contentType == null || !contentType.toLowerCase().contains("text/html")) {
                 ...
                 // Check for specific error messages if not HTML but still not a direct download
            ...
            // Read the HTML content from a successful response (e.g. HTTP_OK)
            String pageContent;
            ...
            } else if (connection.getErrorStream() != null) { // Read error stream for error pages
            ...
            connection.disconnect(); // Disconnect current connection before making a new one
            ...
            // Add more specific error checks from HTML if needed
            ...
            if (confirmToken != null) {
                ...
                // Recursive call with the new confirmation URL
                return attemptDownload(confirmationUrl, fileId, attemptDepth + 1);
            } else {
                // Python script tries confirm='t' as a last resort.
                // This is less common now but can be added if simple confirm fails.
                // For now, if no specific token, assume failure.
                Log.w(TAG, "No confirmation token found on page for ID: " + fileId + ". URL: " + downloadUrl);
                // Log some of the page content for debugging if needed
                // Log.d(TAG, "Page content sample for no-token: " + pageContent.substring(0, Math.min(pageContent.length(), 500)));
                return null;
            }
    ...
    private String readStreamToString(BufferedReader reader) throws java.io.IOException {
        ...
        while ((line = reader.readLine()) != null) {
            stringBuilder.append(line).append("\n"); // Append newline for easier log reading if needed
        }
        reader.close();
    ...
    private String extractFilenameFromContentDisposition(String contentDisposition) {
        ...
        // Try common patterns: filename="...", filename*=UTF-8''...
        Pattern pattern = Pattern.compile("filename\\*?=['\"]?(?:UTF-8'')?([^;"']+)[\"']?", Pattern.CASE_INSENSITIVE);
        Matcher matcher = pattern.matcher(contentDisposition);
        if (matcher.find()) {
            try {
                String filename = matcher.group(1);
                // URL decode because filenames in headers are often URL encoded
                return URLDecoder.decode(filename, StandardCharsets.UTF_8.name());
            } catch (Exception e) {
                Log.w(TAG, "Failed to decode filename from Content-Disposition: " + contentDisposition, e);
                // Fallback to just taking the matched group if decoding fails (e.g. if not actually URL encoded)
